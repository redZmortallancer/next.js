...
pub struct NextDynamicEntries(Vec<NextDynamicEntryAssetVc>);
        ...
        let nodes: Vec<_> = AdjacencyMap::new()
            ...
            .visit(
                entries
                    ...
                    .iter()
                    .copied()
                    .map(|asset| VisitDynamicNode::Internal(asset))
                    .collect::<Vec<_>>(),
                VisitDynamic,
            )
            .await
...
enum VisitDynamicNode {
    Dynamic(NextDynamicEntryAssetVc),
    Internal(AssetVc),
}
        ...
        async move {
            ...
            let referenced_assets = referenced_assets.map(|asset| async move {
                if let Some(next_dynamic_asset) =
                    NextDynamicEntryAssetVc::resolve_from(asset).await?
                {
