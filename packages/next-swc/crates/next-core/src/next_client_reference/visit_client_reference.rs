...
use super::{
    css_client_reference::css_client_reference_asset::CssClientReferenceAssetVc,
    ecmascript_client_reference::ecmascript_client_reference_asset::EcmascriptClientReferenceAssetVc,
};
use crate::next_server_component::server_component_asset::NextServerComponentAssetVc;

#[derive(
    Copy, Clone, Eq, PartialEq, Hash, Serialize, Deserialize, Debug, ValueDebugFormat, TraceRawVcs,
)]
pub struct ClientReference {
    server_component: Option<NextServerComponentAssetVc>,
    ty: ClientReferenceType,
}

impl ClientReference {
    pub fn server_component(&self) -> Option<&NextServerComponentAssetVc> {
...
pub enum ClientReferenceType {
    EcmascriptClientReference(EcmascriptClientReferenceAssetVc),
    CssClientReference(CssClientReferenceAssetVc),
}
                ...
                VisitClientReferenceNodeType::Internal(asset) => {
                    ...
                    let referenced_assets = referenced_assets.map(|asset| async move {
                        if let Some(client_reference_asset) =
                            EcmascriptClientReferenceAssetVc::resolve_from(asset).await?
                        {
                        ...
                        if let Some(css_client_reference_asset) =
                            CssClientReferenceAssetVc::resolve_from(asset).await?
                        {
                            return Ok(VisitClientReferenceNode {
                                server_component: node.server_component,
                                ty: VisitClientReferenceNodeType::ClientReference(
                        ...
                        if let Some(server_component_asset) =
                            NextServerComponentAssetVc::resolve_from(asset).await?
                        {
                            return Ok(VisitClientReferenceNode {
