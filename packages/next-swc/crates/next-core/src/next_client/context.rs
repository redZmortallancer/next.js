...
pub fn get_client_chunking_context(
    ...
    DevChunkingContextVc::builder(
        project_path,
        client_root,
        get_client_chunks_path(client_root, mode),
        get_client_assets_path(client_root, mode),
        environment,
    )
...
#[turbo_tasks::function]
pub fn get_client_chunks_path(client_root: FileSystemPathVc, mode: NextMode) -> FileSystemPathVc {
    match mode {
        NextMode::Development => client_root.join("_next/static/chunks"),
        NextMode::Build => client_root.join("static/chunks"),
    }
}

#[turbo_tasks::function]
pub fn get_client_assets_path(client_root: FileSystemPathVc, mode: NextMode) -> FileSystemPathVc {
    match mode {
        NextMode::Development => client_root.join("_next/static/media"),
        NextMode::Build => client_root.join("static/media"),
    }
}
