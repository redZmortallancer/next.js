...
use turbopack_binding::turbopack::{
    ...
    ecmascript::{magic_identifier, text::TextContentSourceAssetVc, utils::StringifyJs},
...
impl LoaderTreeBuilder {
    ...
    async fn write_component(
        ...
        if let Some(component) = component {
            ...
            let source_asset = SourceAssetVc::new(component).into();
            let reference_ty = Value::new(ReferenceType::EcmaScriptModules(
                EcmaScriptModulesReferenceSubType::Undefined,
            ));

            let inner_asset = match &self.server_component_transition {
                ServerComponentTransition::Transition(transition) => {
                    transition.process(source_asset, self.context, reference_ty)
                }
                ServerComponentTransition::TransitionName(transition_name) => self
                    .context
                    .with_transition(transition_name.as_str())
                    .process(source_asset, reference_ty),
            };

            self.inner_assets
                .insert(format!("COMPONENT_{i}"), inner_asset);
        }
        Ok(())
    }
        ...
        match manifest {
            MetadataItem::Static { path } => {
                ...
                self.inner_assets.insert(
                    inner_module_id,
                    StaticModuleAssetVc::new(SourceAssetVc::new(path).into(), self.context.into())
                        .into(),
                );
