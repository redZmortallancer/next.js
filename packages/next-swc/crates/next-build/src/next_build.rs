...
use crate::{
    ...
    manifests::{
        ...
        ClientReferenceManifest, FontManifest, MiddlewaresManifest, NextFontManifest,
        PagesManifest, ReactLoadableManifest, ServerReferenceManifest,
    },
    next_app::{
...
pub(crate) async fn next_build(options: TransientInstance<BuildOptions>) -> Result<CompletionVc> {
    ...
    let page_ssr_entries = page_entries
        ...
        .iter()
        .copied()
        .map(|entry| async move { Ok(entry.await?.ssr_asset) })
        .try_join()
        .await?;
    ...
    let (
        Some(rsc_chunking_context),
        Some(ssr_chunking_context),
    ) = (
    ...
    let mut client_reference_manifest = ClientReferenceManifest::default();
    let client_reference_manifest_path = node_root.join("server/client-reference-manifest.json");
